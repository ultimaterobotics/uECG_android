...
public class pixel_UI {
    ...
    public class GraphButton
    {
        ...
        public SymbolType draw_symbol = SymbolType.none;
        public GBType type;
        ...
        public void draw(Canvas img, int draw_mask)
        {
            ...
            if(draw_symbol == SymbolType.cross)
            {
                pnt.setARGB(255, cr, cg, cb);
                pnt.setStyle(Paint.Style.STROKE);
            ...
            if(draw_symbol == SymbolType.arrowUp)
            {
                pnt.setARGB(255, cr, cg, cb);
                pnt.setStyle(Paint.Style.STROKE);
                img.drawLine((left + right)/2, bottom, (left + right)/2, top, pnt);
                img.drawLine((left + right)/2, top, left+5, (top+bottom)/2, pnt);
                img.drawLine((left + right)/2, top, right-5, (top+bottom)/2, pnt);
            }
            if(draw_symbol == SymbolType.arrowDown)
            {
                pnt.setARGB(255, cr, cg, cb);
                pnt.setStyle(Paint.Style.STROKE);
                img.drawLine((left + right)/2, bottom, (left + right)/2, top, pnt);
                img.drawLine((left + right)/2, bottom, left+5, (top+bottom)/2, pnt);
                img.drawLine((left + right)/2, bottom, right-5, (top+bottom)/2, pnt);
            }
            if(draw_text > 0)
            {
            ...
            if(type == GBType.progress) {
                pnt.setARGB(255, cr, cg, cb);
                pnt.setStyle(Paint.Style.FILL);
                int right_p = (int)((x + w*progress_percent*0.01) * width);
                img.drawRect(left, top, right_p, bottom, pnt);
            }
        }
        ...
        public void process_touch(int touch_type, float tx, float ty, float sx, float sy)
        {
            ...
            if((type == GBType.toggle || type == GBType.checkbox) && touch_type == 1)
            {
                ...
                if(ms - pressed_time > 150) //prevent too close events
                {
            ...
            if(type == GBType.touch || type == GBType.symbol && touch_type == 1)
            {
                long ms = System.currentTimeMillis();
                if(ms - pressed_time > 150) //prevent too close events
                    touch_callback.callback(1, 0, 0);
