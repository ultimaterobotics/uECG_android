...
public class MainActivity extends Activity {
    ...
    float charts_time_scroll = 0;
    float charts_time_scroll_base = 0;

    int cur_mark_value = 100;

    String connection_state_message = "";
    ...
    pixel_UI.GraphButton bt_capture_image;

    pixel_UI.GraphButton bt_record;
    pixel_UI.GraphButton bt_mark_data;
    pixel_UI.GraphButton bt_mark_up;
    pixel_UI.GraphButton bt_mark_down;
...
//            fw_file
    ...
    void create_buttons()
    {
        ...
        close.text = "";
        close.toggled_text = "";
        close.type = pixel_UI.GBType.symbol;
        close.draw_mask = 0x01;
        close.draw_symbol = pixel_UI.SymbolType.cross;
...
//                                        save_on = 1;
        ...
        bt_mark_data.text = "Mark " + cur_mark_value;
        ...
        bt_mark_data.set_callback(new pixel_UI.SimpleCallback() {
            ...
            public void callback(int act, float dx, float dy) {
                ...
                intent.putExtra("mark_value", cur_mark_value);
                        intent.addCategory(Intent.CATEGORY_DEFAULT);
                sendBroadcast(intent);
...
/*                Timer tm = new Timer();
        ...
        bt_mark_up = pixUI.new GraphButton();
        bt_mark_up.set_coords(bt_mark_data.x + bt_mark_data.w, bt_mark_data.y - 0.02f, 0.05f, 0.04f);
        bt_mark_up.set_color_normal(30, 200, 70);
        bt_mark_up.set_color_toggled(130, 100, 70);
        bt_mark_up.text_size = 30;
        bt_mark_up.text = "";
        bt_mark_up.type = pixel_UI.GBType.symbol;
        bt_mark_up.draw_symbol = pixel_UI.SymbolType.arrowUp;
        bt_mark_up.draw_mask = 0x0;

        bt_mark_up.set_callback(new pixel_UI.SimpleCallback() {
            @Override
            public void callback(int act, float dx, float dy) {
                cur_mark_value++;
                bt_mark_data.text = "Mark " + cur_mark_value;
            } });
        buttons.add(bt_mark_up);

        bt_mark_down = pixUI.new GraphButton();
        bt_mark_down.set_coords(bt_mark_data.x + bt_mark_data.w, bt_mark_data.y + 0.03f, 0.05f, 0.04f);
        bt_mark_down.set_color_normal(30, 200, 70);
        bt_mark_down.set_color_toggled(130, 100, 70);
        bt_mark_down.text_size = 30;
        bt_mark_down.text = "";
        bt_mark_down.type = pixel_UI.GBType.symbol;
        bt_mark_down.draw_symbol = pixel_UI.SymbolType.arrowDown;
        bt_mark_down.draw_mask = 0x0;

        bt_mark_down.set_callback(new pixel_UI.SimpleCallback() {
            @Override
            public void callback(int act, float dx, float dy) {
                cur_mark_value--;
                bt_mark_data.text = "Mark " + cur_mark_value;
            } });
        buttons.add(bt_mark_down);
...
//        buttons.add(bt_range_12hr);
        ...
        float btns_x = 0.05f;
        float btns_x_step = 0.15f;
        bt_BPM = pixUI.new GraphButton();
        bt_BPM.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        ...
        bt_HRV.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        ...
        bt_GSR.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        ...
        bt_steps.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        ...
        bt_accel.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        ...
        bt_temp = pixUI.new GraphButton();
        bt_temp.set_coords(btns_x, 0.01f, 0.15f, 0.06f);
        btns_x += btns_x_step;
        bt_temp.set_color_normal(30, 200, 70);
        bt_temp.set_color_toggled (30, 200, 70);
        bt_temp.text_size = 30;
        bt_temp.text = "T";
        bt_temp.type = pixel_UI.GBType.checkbox;
        bt_temp.chbox_size = 0.04f;
        bt_temp.draw_mask = 0x04;
        bt_temp.pressed = 1;
        bpm_processor.set_chart_state(BPM_processor.chart_ID_T, bt_temp.pressed);
        bt_temp.draw_rect = 0;

        bt_temp.set_callback(new pixel_UI.SimpleCallback() {
            @Override
            public void callback(int act, float dx, float dy) {
                bpm_processor.set_chart_state(BPM_processor.chart_ID_T, bt_temp.pressed);
            }});
...
//                    ecg_processor.set_screen_time( 2.5f / dx);
        ...
        pixel_UI.GraphButton charts_scroll = pixUI.new GraphButton();
        charts_scroll.set_coords(0, 0.05f, 1, 0.9f);
        charts_scroll.draw_rect = 0;
        charts_scroll.draw_text = 0;
        charts_scroll.type = pixel_UI.GBType.scroll;
        charts_scroll.draw_mask = 0x04;
        charts_scroll.set_callback(new pixel_UI.SimpleCallback() {
            float scroll_prev_dx = 0;
            @Override
            public void callback(int act, float dx, float dy) {
                if(act == 1 || act == 2) {
                    float rng = bpm_processor.get_range();
                    charts_time_scroll = charts_time_scroll_base + rng*1.0f*dx;
                    if (charts_time_scroll < 0) charts_time_scroll = 0;
                    if (charts_time_scroll > 3600*48) charts_time_scroll = 3600*48;
                    if (act == 1) {
                        charts_time_scroll_base = charts_time_scroll;
                    }
                    bpm_processor.set_charts_offset(charts_time_scroll);
                    //ecg_processor.set_draw_shift(time_scroll);
                }
                if(act == 3)
                {
                    ...
                    if(chg > 0) {
                        ...
                        rng *= 0.97;
                        if (rng < 90) rng = 90;
                        bpm_processor.set_range(rng);
                    }
                    if(chg < 0) {
                        float rng = bpm_processor.get_range();
                        rng *= 1.03;
                        if (rng > 3600*48) rng = 3600*48;
                        bpm_processor.set_range(rng);
                    }
//                    ecg_processor.set_screen_time( 15.0f / (1.0f + 4.0f*dx) );
//                    ecg_processor.set_screen_time( 2.5f / dx);
        ...
        bt_capture_image = pixUI.new GraphButton();
        bt_capture_image.set_coords(0.15f, 0.25f, 0.3f, 0.06f);
        bt_capture_image.set_color_normal(30, 200, 70);
        bt_capture_image.set_color_toggled(130, 100, 70);
        bt_capture_image.text_size = 30;
        bt_capture_image.text = "Snapshot 60s";
        bt_capture_image.type = pixel_UI.GBType.toggle;
        bt_capture_image.draw_mask = 0x01;

        bt_capture_image.set_callback(new pixel_UI.SimpleCallback() {
            @Override
            public void callback(int act, float dx, float dy) {
                Intent intent = new Intent("uECG_SERVICE_SIGNAL");
                intent.putExtra("signal", ble_uecg_service.UECG_MAKE_SNAPSHOT_REQUEST);
                intent.putExtra("snapshot_length", 60);
                intent.addCategory(Intent.CATEGORY_DEFAULT);
                sendBroadcast(intent);
                ...
                TimerTask task = new TimerTask() {
                    public void run() {
                        bt_capture_image.pressed = 0;
                    }
                };
                tm.schedule(task , 200);

            } });
        buttons.add(bt_capture_image);
    }
    ...
    int data_push_ecg(Bundle b)
    {
        ...
        if(bt_record.pressed > 0)
        {
            ...
            bt_mark_up.draw_mask = 0x01;
            bt_mark_down.draw_mask = 0x01;
            ...
            bt_mark_up.draw_mask = 0x00;
            bt_mark_down.draw_mask = 0x00;
        }
    ...
    int data_push_imu_rr(Bundle b)
    {
        ...
        if(has_RR > 0)
        {
            ...
            calc_RMSSD_5m = (int)b.getFloat("RR_rmssd5m");
        }
//        bpm_processor.push_data(dev_BPM, dev_steps, dev_skin, rr_processor.get_hrv_parameter(), dev_ax, dev_ay, dev_az);
        bpm_processor.push_data(dev_BPM, dev_steps, dev_skin, calc_RMSSD_5m, dev_ax, dev_ay, dev_az, dev_T);
        return 1;
    }
...
//                        startForegroundService(ble_service_intent);
                    ...
                    if(bat_v < 2100)
                    {
                        if(connection_state_message.length() < 1) connection_state_message = "Waiting for uECG device";
                        lbl_message.text = connection_state_message;// "Searching for uECG device...";
                    }
                    ...
                    lbl_SDRR.text = "RMSSD: " + calc_RMSSD_5m;// dev_SDRR; //dev_SDNN;
...
//                parse_result(b.getByteArray("data"), b.getInt("type"), b.getInt("RSSI"), b.getString("MAC"));
        ...
        BroadcastReceiver ecg_service_info = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if(sig == ble_uecg_service.UECG_CONNECT_UPDATE)
                {
                    Bundle b = intent.getBundleExtra("conn_state");
                    connection_state_message = b.getString("message");
                }
